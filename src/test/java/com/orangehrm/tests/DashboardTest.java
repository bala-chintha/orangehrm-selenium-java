...
public class DashboardTest extends BaseTest {
    ...
    public void shouldDisplayDashboardHeading() {
        Assert.assertTrue(dashboardPage.isDashboardHeadingVisible(), "Dashboard heading (h6) should be visible");
    }
    ...
    public void shouldNavigateToPimViaSidebar() {
        ...
        Assert.assertTrue(getDriver().getCurrentUrl().contains("viewEmployeeList"), "URL should contain 'viewEmployeeList' after clicking PIM sidebar link");
    }
    ...
    public void shouldNavigateToLeaveViaSidebar() {
        ...
        Assert.assertTrue(getDriver().getCurrentUrl().contains("viewLeaveList"), "URL should contain 'viewLeaveList' after clicking Leave sidebar link");
    }
    ...
    public void shouldNavigateToAdminViaSidebar() {
        ...
        Assert.assertTrue(getDriver().getCurrentUrl().contains("viewSystemUsers"), "URL should contain 'viewSystemUsers' after clicking Admin sidebar link");
    }

    @Test(groups = {"smoke", "dashboard"}, description = "Logout should redirect back to the login page")
    public void shouldLogoutSuccessfully() {
        dashboardPage.logout();
        Assert.assertTrue(getDriver().getCurrentUrl().contains("auth/login"), "URL should contain 'auth/login' after logout");
    }
}
