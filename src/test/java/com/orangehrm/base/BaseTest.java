...
public class BaseTest {
    ...
    public static final ThreadLocal<Boolean> isRetryAttempt = ThreadLocal.withInitial(() -> false);
    ...
    public void setUpDriver() {
        if (Boolean.TRUE.equals(isRetryAttempt.get())) {
            System.out.println("[RETRY] Fresh browser session for retry attempt");
            WebDriver old = driverHolder.get();
            if (old != null) {
                try {
                    old.quit();
    ...
    protected String getLoggedInEmployeeName() {
        return pom().getHeaderComponent().captureLoggedInUserFirstName();
    }
    ...
    protected void loginAsAdmin() {
        ...
        pom().getLoginPage().login(config.getAdminUsername(), config.getAdminPassword());
